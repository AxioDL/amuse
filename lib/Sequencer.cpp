...
Sequencer::Sequencer(Engine& engine, const AudioGroup& group, int groupId, const SFXGroupIndex* sfxGroup, Submix* smx)
...
std::shared_ptr<Voice> Sequencer::ChannelState::keyOn(uint8_t note, uint8_t velocity)
{
    ...
    std::list<std::shared_ptr<Voice>>::iterator ret =
        m_parent.m_engine._allocateVoice(m_parent.m_audioGroup, m_parent.m_groupId, 32000.0, true, false, m_parent.m_submix);
    if (*ret)
    {
        m_chanVoxs[note] = *ret;
