...
void BooBackendVoice::VoiceCallback::preSupplyAudio(boo::IAudioVoice&, double dt) { m_parent.m_clientVox.preSupplyAudio(dt); }
...
BooBackendVoice::BooBackendVoice(boo::IAudioSubmix& submix, Voice& clientVox, double sampleRate, bool dynamicPitch)
: m_clientVox(clientVox), m_cb(*this), m_booVoice(submix.allocateNewMonoVoice(sampleRate, &m_cb, dynamicPitch))
{
}
...
void BooBackendSubmix::setChannelGains(const float gains[8]) { m_booSubmix->setChannelGains(gains); }

std::unique_ptr<IBackendVoice> BooBackendSubmix::allocateVoice(Voice& clientVox, double sampleRate, bool dynamicPitch)
{
...
std::unique_ptr<IBackendVoice> BooBackendVoiceAllocator::allocateVoice(Voice& clientVox, double sampleRate, bool dynamicPitch)
{
