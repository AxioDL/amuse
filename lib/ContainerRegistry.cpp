            ...
            if (type == 0x41475343)
            {
                ...
                if (fread(testBuf, 1, 6, fp) == 6)
                {
                    if (!memcmp(testBuf, "Audio/", 6))
                    {
                        ...
                        ret.emplace_back(std::move(name), IntrusiveAudioGroupData{proj.release(), projLen, pool.release(),
                                                                                  poolLen, sdir.release(), sdirLen,
                ...
                if (fread(testBuf, 1, 4, fp) == 4)
                {
                    if (amuse::SBig(*reinterpret_cast<uint32_t*>(testBuf)) == 0x1)
                    {
                        ...
                        if (projSz && poolSz && sdirSz && sampSz)
                        {
                            ...
                            ret.emplace_back(std::move(name), IntrusiveAudioGroupData{proj.release(), projSz, pool.release(),
                                                                                      poolSz, sdir.release(), sdirSz,
...
static std::vector<std::pair<SystemString, IntrusiveAudioGroupData>> LoadRS1N64(FILE* fp)
{
    ...
    if (dataSeg)
    {
        ...
        ret.emplace_back(_S("Group"), IntrusiveAudioGroupData{proj.release(), projSz, pool.release(), poolSz, sdir.release(),
                                                              sdirSz, samp.release(), sampSz, false, N64DataTag{}});
    }
...
static std::vector<std::pair<SystemString, IntrusiveAudioGroupData>> LoadBFNPC(FILE* fp)
{
    ...
    if (fread(&fstOff, 1, 4, fp) == 4 && fread(&fstSz, 1, 4, fp) == 4)
    {
        if (fstOff + fstSz <= endPos)
        {
            ...
            ret.emplace_back(_S("Group"),
                             IntrusiveAudioGroupData{proj.release(), projSz, pool.release(), poolSz, sdir.release(), sdirSz,
                                                     samp.release(), sampSz, true, PCDataTag{}});
        }
    }
...
static std::vector<std::pair<SystemString, IntrusiveAudioGroupData>> LoadBFNN64(FILE* fp)
{
    ...
    if (dataSeg)
    {
        ...
        ret.emplace_back(_S("Group"), IntrusiveAudioGroupData{proj.release(), projSz, pool.release(), poolSz, sdir.release(),
                                                              sdirSz, samp.release(), sampSz, true, N64DataTag{}});
    }
                                                                                               ...
                                                                                               Type& typeOut)
{
    ...
    if (IsChunkExtension(path, dot))
    {
        ...
        if (*reinterpret_cast<uint32_t*>(sdir.get() + 8) == 0x0)
            ret.emplace_back(_S("Group"),
                             IntrusiveAudioGroupData{proj.release(), projLen, pool.release(), poolLen, sdir.release(), sdirLen,
                                                     samp.release(), sampLen, GCNDataTag{}});
        else if (sdir[9] == 0x0)
            ret.emplace_back(_S("Group"),
                             IntrusiveAudioGroupData{proj.release(), projLen, pool.release(), poolLen, sdir.release(), sdirLen,
                                                     samp.release(), sampLen, false, N64DataTag{}});
        else
            ret.emplace_back(_S("Group"),
                             IntrusiveAudioGroupData{proj.release(), projLen, pool.release(), poolLen, sdir.release(), sdirLen,
                                                     samp.release(), sampLen, false, PCDataTag{}});
